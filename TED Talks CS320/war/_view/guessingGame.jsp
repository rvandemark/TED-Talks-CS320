...
<html>
	...
	<body>
		<form action="${pageContext.servletContext.contextPath}/guessingGame" method="post">
			...
			<c:if test="${! empty game}">
				<c:if test="${game.done}">
					<c:if test="${game.guess<1}">
						<div>
							The number you are thinking of is 0, which is invalid and you clearly can't read the instructions ;)
						</div>
					</c:if>
					<c:if test="${game.guess>=1 && game.guess<=100}">
						<div>
